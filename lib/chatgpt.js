...
const { OPENAI_API_KEY, PROMPT, NODE_ENV } = process.env;
...
const askChatGPT = async ({ prompt, retries = 0 }) => {
  try {
    ...
    try {
      ...
      console.warn("=> Repairing JSON:", clean);

      try {
        return jsonrepair(clean);
      } catch (error) {
        console.error("=> Failed to repair JSON:", clean);
        return "";
      }
    }
  } catch (error) {
    ...
    if (statusCode === 429) {
      console.error("Too many requests to OpenAI API, retrying in 5 seconds");
      if (retries < 3) {
        await new Promise((resolve) => setTimeout(resolve, 5000));
        return askChatGPT({ prompt, retries: retries + 1 });
      } else {
        console.error("Too many retries to OpenAI API, aborting");
      }
    }
...
const parseProperties = async (body) => {
  ...
  //   console.log("=> Parsing properties from:", content);
  // }
  ...
  if (contentLength > maxContentLength) {
    console.warn(
      `Truncating prompt from ${contentLength} to ${maxContentLength}`
    );
